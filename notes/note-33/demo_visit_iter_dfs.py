...
def dump_tree(node, width=96):
    """
    format the tree with good spacing (children diagonally below parent), 
    with root at node, set into line of length width
    -- kind of crude, but good enough for debugging
    """
    glue = (width-2) // 2                   # we know using 2 digit keys
    fill = ' ' * glue                       # figure out filler on each side of node
    if node:
